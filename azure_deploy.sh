...
z network nsg rule create --resource-group "$RG_NAME" --nsg-name "$NSG_NAME" --priority 108 --name SSL --protocol tcp --source-port-ranges '*' --description "SSL" --direction 'inbound' --access 'allow' --destination-port-ranges 18091-18096 --source-address-prefixes 'Internet' --destination-address-prefixes '*'
...
az vmss create --name $VMSS_NAME_1 --resource-group $RG_NAME --image $VMSS_VM_IMAGE_1 --instance-count $VMSS_INSTANCE_COUNT_1 --admin-username $VMSS_INSTANCE_USERNAME --nsg $NSG_NAME --tags $NSG_TAGS --vm-sku $VMSS_INSTANCE_SIZE_1 --disable-overprovision --data-disk-sizes-gb $VMSS_INSTANCE_DISK_SIZE_1 --vm-domain-name "$VMSS_NAME_1-$UNIQUE_STRING" --public-ip-per-vm --public-ip-address-dns-name "$VMSS_NAME_1-$UNIQUE_STRING"

az vmss extension set --vmss-name $VMSS_NAME_1 --publisher Microsoft.OSTCExtensions  --version 1.4 --name CustomScriptForLinux --resource-group $RG_NAME --settings "{ \"fileUris\": [\"https://cbhelpers.blob.core.windows.net/extensions/server.sh\",\"https://cbhelpers.blob.core.windows.net/extensions/util.sh\"], \"commandToExecute\": \"bash server.sh $CB_VERSION $CB_ADMIN_USER $CB_ADMIN_PASS $UNIQUE_STRING $RG_LOCATION\" }" --force-update

az network lb probe create --lb-name ${VMSS_NAME_1}LB --name admin-ui-probe --port 8091 --resource-group $RG_NAME --protocol TCP
az network lb rule create --probe-name admin-ui-probe --lb-name ${VMSS_NAME_1}LB --name admin-ui --frontend-port 8091 --backend-port 8091 --resource-group $RG_NAME --protocol TCP --load-distribution SourceIP

az vmss create --name $VMSS_NAME_2 --resource-group $RG_NAME --image $VMSS_VM_IMAGE_2 --instance-count $VMSS_INSTANCE_COUNT_2 --admin-username $VMSS_INSTANCE_USERNAME --nsg $NSG_NAME --tags $NSG_TAGS --vm-sku $VMSS_INSTANCE_SIZE_2 --disable-overprovision --public-ip-address-dns-name $VMSS_NAME_2 --public-ip-per-vm

az vmss extension set --vmss-name $VMSS_NAME_2 --publisher Microsoft.OSTCExtensions --version 1.4 --name CustomScriptForLinux --resource-group $RG_NAME --settings "{ \"fileUris\": [\"https://cbhelpers.blob.core.windows.net/extensions/syncGateway.sh\",\"https://cbhelpers.blob.core.windows.net/extensions/util.sh\"], \"commandToExecute\": \"bash syncGateway.sh $SG_VERSION\" }" --force-update

az network lb probe create --lb-name ${VMSS_NAME_2}LB --name admin-ui-probe --port 4985 --resource-group $RG_NAME --protocol TCP
az network lb rule create --probe-name admin-ui-probe --lb-name ${VMSS_NAME_2}LB --name admin-ui --frontend-port 4985 --backend-port 4985 --resource-group $RG_NAME --protocol TCP --load-distribution SourceIP

az network lb probe create --lb-name ${VMSS_NAME_2}LB --name interface-probe --port 4984 --resource-group $RG_NAME --protocol TCP
az network lb rule create --probe-name interface-probe --lb-name ${VMSS_NAME_1}LB --name interface --frontend-port 4984 --backend-port 4984 --resource-group $RG_NAME --protocol TCP --load-distribution SourceIP
